...
local coreGui = game.CoreGui or game:GetService("CoreGui")
	...
	for index,player in pairs(playerService:GetChildren()) do
		if player ~= localPlayer and teamCheck(player) then 
			...
			if character:FindFirstChild("Humanoid") and character:FindFirstChild("HumanoidRootPart") and character.Humanoid.Health > 0 then 
				...
				if (magnitude < dist and magnitude < circleSettings.Radius) then
					dist = magnitude
...
local function gunMod(num, value)
...
mainLoop = runService.RenderStepped:Connect(function()
    ...
    local character = getClosestPlayer().Character or getClosestPlayer().CharacterAdded:Wait()
		...
		if aimSettings.Aimbot and aimSettings.isAiming and IsVisible(getClosestPlayer().Character, {camera, localCharacter, Workspace.Debris, Workspace.Ray_Ignore, getClosestPlayer(), Workspace.Map.Ignore}) then
			aimAt(position)
		end
	else
	...
	if silentSettings.HeadChance or 0 <= chance then
	silentSettings.AimPart = "HumanoidRootPart"
	elseif silentSettings.BodyChance or 0 >= chance then
	silentSettings.AimPart = "Head"
	else
	silentSettings.AimPart = "HumanoidRootPart"
	end
end)
...
local Window = OrionLib:MakeWindow({Name = "luaware", HidePremium = true, SaveConfig = false, ConfigFolder = "luaware", IntroEnabled = true, IntroText = "luaware", IntroIcon = "rbxassetid://11212490886"})
...
ModTab:AddLabel("Equip weapon again if not working")
...
Notify("Select a skin to swap/replace.")
return
	end
...
Notify("Welcome, "..localPlayer.DisplayName..".")
